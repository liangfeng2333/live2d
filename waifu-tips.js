...
function loadWidget(config) {
	...
	(function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主页
			...
			if (now > 5 && now <= 7) text = "早上好！今天也是元气满满的一天哦！", "早上好，今天要充满干劲哟！";
			else if (now > 7 && now <= 11) text = "上午好！工作顺利嘛，不要久坐，多起来走动走动哦！";
			else if (now > 11 && now <= 13) text = "中午好！现在是恰午餐时间！";
			else if (now > 13 && now <= 17) text = "午后很容易犯困呢，今天的目标完成了吗？", "午后总是很慵懒呢";
			else if (now > 17 && now <= 19) text = "傍晚了！记得那日在夕阳下的奔跑，那是我逝去的青春…", "傍晚很适合回忆呢";
			else if (now > 19 && now <= 21) text = "晚上好，今天过得怎么样？", "总结一下今天的经历吧";
			else if (now > 21 && now <= 23) text = ["已经这么晚了呀，早点休息吧，晚安～", "快去睡觉呀！", "晚安~", "深夜时要爱护眼睛呀！"];
			else text = "你是夜猫子呀？这么晚还不睡觉，明天起的来嘛？", "这时候的好孩子已经躺在床上了";
		...
		if (useCDN) {
			...
			fetch(`${apiPath}rand_textures/?id=${modelId}-${modelTexturesId}`)
				...
				.then(result => {
					...
					else loadModel(modelId, result.textures.id, "我的新衣服好看嘛？", "变身！", "Duang~");
				});
		}
	}
